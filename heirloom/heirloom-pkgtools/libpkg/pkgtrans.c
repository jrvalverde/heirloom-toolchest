...
/*	from OpenSolaris "pkgtrans.c	1.29	07/01/11 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)pkgtrans.c	1.8 (gritter) 2/25/07
 */
    ...
    keystore_handle_t keystore, char *keystore_alias)
{
	...
	if (ids_name) {
		...
		if (close(mkstemp(template)) < 0)
			...
			unlink(template);
			tmpdir = srcdev.dirname = strdup(template);
		}
...
{
	char	template[] = "/var/tmp/wdXXXXXX";
	FILE	*fp;
	...
	if (close(mkstemp(tmp_file)) < 0 ||	/* temporary file name */
			(list_fd = open(tmp_file, O_RDWR | O_CREAT, S_IRUSR|S_IWUSR)) == -1) {
		progerr(pkg_gt(ERR_TRANSFER));
...
pkgxfer(char *srcinst, int options)
{
	...
	if (options & PT_ODTSTREAM) {
		...
		(void) mgets(line, 128);
		curpartcnt = -1;
		if (sscanf(line, "%s %d %d %[ 0-9]", (char *)&pkgname, &nparts,
