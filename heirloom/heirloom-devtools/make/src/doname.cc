...
/*	from OpenSolaris "doname.cc	1.115	06/12/12"	*/
...
#if defined (__sun) 	/* linux does not support STREAMS */
#	include <stropts.h>
#endif
...
try_again:
	...
	case build_dont_know:
		...
		fatal(dontknow, target->string_mb);
		break;
	}
	return build_failed;
}
 ...
 */
		...
		if ((command == NULL) &&
			...
			if (build_unconditional || out_of_date) {
				...
				if (debug_level > 0) {
					printf("%*sBuilding %s using .DEFAULT because it is out of date\n",
						      recursion_level,
						      "",
...
r_command:
	...
	if ((result == build_ok) && check_auto_dependencies(target,
							    ...
							    automatics)) {
		if (debug_level > 0) {
			printf("%*sTarget `%s' acquired new dependencies from build, rechecking all dependencies\n",
				      recursion_level,
				      "",
				      true_target->string_mb);
		}
 ...
 */
				...
				rewrite_statefile =
				  command_changed =
				    true;
				if (debug_level > 0) {
					printf("Target %s rebuilt because dependency %s does not exist\n",
						     true_target->string_mb,
			...
			if (debug_level > 1) {
				...
				if (dependency->name->stat.time > line->body.line.dependency_time) {
					printf("%*sDate-dependencies(%s) set to %s\n",
						      recursion_level,
						      "",
						      true_target->string_mb,
				...
				*out_of_date_tail = ALLOC(Chain);
				...
				if (debug_level > 0) {
					if (dependency->name->stat.time == file_max_time) {
						printf("%*sBuilding %s because %s does not exist\n",
							      recursion_level,
							      "",
							      true_target->string_mb,
							      dependency->name->string_mb);
					} else {
						printf("%*sBuilding %s because it is out of date relative to %s\n",
							      recursion_level,
							      "",
							      true_target->string_mb,
							      dependency->name->string_mb);
...
/* azv */
		...
		do {
			...
			if (p != start) {
				...
				if ((debug_level > 0) &&
				    (first_member == NULL)) {
					printf("%*sDynamic dependency `%s' for target `%s'\n",
						      recursion_level,
						      "",
						      dependency->name->string_mb,
						      true_target->string_mb);
...
execute_serial(Property line)
{
	...
	Cmd_line		rule, cmd_tail = NULL, command = NULL;
	...
	for (rule = command; rule != NULL; rule = rule->next) {
		...
		if (result == build_failed) {
			if (silent || rule->silent) {
				...
				SEND_MTOOL_MSG(
					job_result_msg->appendOutput(AVO_STRDUP("The following command caused the error:"));
					job_result_msg->appendOutput(AVO_STRDUP(rule->command_line->string_mb));
				);
			}
...
append_job_result_msg(Avo_MToolJobResultMsg *job_result_msg)
{
	...
	if (stdout_file != NULL) {
		...
		if (fp == NULL) {
			...
			warning("fopen() of stdout_file failed. Output may be lost");
			return;
		}
		...
		warning("Internal stdout_file variable shouldn't be NULL. Output may be lost");
	}
}
...
check_state(Name temp_file_name)
{
	...
	if (temp_file_name != NULL) {
		unlink(temp_file_name->string_mb);
	}
}
...
build_command_strings(Name target, register Property line)
{
	...
	SETVAR(less_name, tmp_name, false);
	...
	SETVAR(percent_name, tmp_name, false);
...
/* new one trying to reuse Cmd_lines */
			...
			if (keep_state &&
				...
				if (debug_level > 0) {
					if (used->command_line != NULL
					    ...
					    '\0') {
						printf("%*sBuilding %s because new command \n\t%s\n%*sdifferent from old\n\t%s\n",
							      recursion_level,
							      "",
							      target->string_mb,
	...
	if (used != NULL) {
		*insert = NULL;
		if (keep_state &&
			...
			if (debug_level > 0) {
				printf("%*sBuilding %s because new command shorter than old\n",
					      recursion_level,
					      "",
					      target->string_mb);
			}
	...
	if (new_command_longer &&
	    ...
	    keep_state) {
		if (debug_level > 0) {
			printf("%*sBuilding %s because new command longer than old\n",
				      recursion_level,
				      "",
				      target->string_mb);
		}
	...
	SETVAR(c_at, (Name) NULL, false);
	SETVAR(star, (Name) NULL, false);
	SETVAR(less_name, (Name) NULL, false);
	SETVAR(percent_name, (Name) NULL, false);
	SETVAR(query, (Name) NULL, false);
        if (query_list != NULL) {
        	delete_query_chain(query_list);
        }
	SETVAR(hat, (Name) NULL, false);
        if (hat_list != NULL) {
...
sccs_get(register Name target, register Property *command)
{
	...
	/* For sccs, we need to chase symlinks. */
		...
		if (sym_link_depth++ > 90) {
			fatal("Can't read symbolic link `%s': Number of symbolic links encountered during path name traversal exceeds 90.",
			      target->string_mb);
		}
	...
	if ((!target->has_complained &&
		...
		*command = line;
		if (sccs_time > target->stat.time) {
			...
			line->body.line.is_out_of_date = true;
			if (debug_level > 0) {
				printf("%*sSccs getting %s because s. file is younger than source file\n",
					      recursion_level,
					      "",
					      target->string_mb);
			}
...
set_locals(register Name target, register Property old_locals)
{
	...
	for (conditional = get_prop(target->prop, conditional_prop), i = 0;
		...
		if (debug_level > 1) {
			printf("%*sActivating conditional value: ",
				      recursion_level,
				      "");
		}
		...
		if ((conditional->body.conditional.name != virtual_root) ||
			...
			SETVAR(conditional->body.conditional.name,
...
reset_locals(register Name target, register Property old_locals, register Property conditional, register int index)
{
	...
	if (conditional->body.conditional.name == virtual_root) {
		SETVAR(virtual_root, getvar(virtual_root), false);
	}
	if (debug_level > 1) {
		if (old_locals[index].body.macro.value != NULL) {
			printf("%*sdeactivating conditional value: %s= %s\n",
				      recursion_level,
				      "",
			...
			printf("%*sdeactivating conditional value: %s =\n",
				      recursion_level,
				      "",
				      conditional->body.conditional.name->
				      string_mb);
...
#ifdef linux
		signal(SIGUSR1, Avo_PollResultsAction_Sigusr1Handler);
#else
		sigset(SIGUSR1, Avo_PollResultsAction_Sigusr1Handler);
#endif
